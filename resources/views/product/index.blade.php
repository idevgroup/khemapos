...
<section class="content">
    ...
    @include('product.partials.print_label')
</section>
<!-- /.content -->
...
$(document).on('click', '#printlbl-selected', function () {
var selected_rows = getSelectedRows();
if (selected_rows.length > 0) {
    $.ajax({
        method: 'GET',
        url: '/labels/load-product-selected',
        dataType: 'html',
        data: {product_id: selected_rows, row_count: selected_rows.length},
        success: function (result) {
            $('#product-selected').html(result);
        },
    });
