...
class LabelsController extends Controller {
    ...
    public function loadViewProductSelected(Request $request) {
        if ($request->ajax()) {
            $business_id = $request->session()->get('user.business_id');
            $product_id = $request->get('product_id', false);
            ...
            $products = [];
             ...
             $products = $this->productUtil->getDetailFromProducts($business_id, $product_id);
          
            $barcode_settings = Barcode::where('business_id', $business_id)
                    ->orWhereNull('business_id')
                    ->pluck('name', 'id');
            $index = 0;
            return view('labels.partials.modal_print_label')
                            ->with(compact('products', 'barcode_settings','index'));
        }
    }
